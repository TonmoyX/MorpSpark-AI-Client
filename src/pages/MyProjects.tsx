                ...
                <div className='py-10 min-h-[80vh]'>
                    ...
                    <div className='flex flex-wrap gap-3.5'>
                        {projects.map((project) => (
                            <div onClick={()=> navigate(`/projects/${project.id}`)} key={project.id} className='relative group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg p-4 overflow-hidden shadow-md group hover:shadow-indigo-700/30 hover:border-indigo-600 transition-all duration-300'>
                                ...
                                <div className='p-4 text-white bg-linear-180 from-transparent group-hover:from-indigo-950 to-transparent transition-colors'>
                                    ...
                                    <div className='flex items-center gap-2 group-hover:opacity-100 transition-opacity'>
                                            <button onClick={()=> navigate(`/preview/${project.id}`)} className='text-sm bg-indigo-600 hover:bg-indigo-700 text-white py-1 px-3 rounded-md transition-colors'>
                                                Preview
                                            </button>
                                            <button onClick={()=> navigate(`/projects/${project.id}`)} className='text-sm bg-indigo-600 hover:bg-indigo-700 text-white py-1 px-3 rounded-md transition-colors'>
                                                Open
                                            </button>
                                        </div>
                                    </div>
