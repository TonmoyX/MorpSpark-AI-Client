...
import { ArrowBigDown, EyeIcon, EyeOffIcon, Fullscreen, LaptopIcon, Loader2Icon, MessageSquareIcon, SaveIcon, SmartphoneIcon, TabletIcon, XIcon } from "lucide-react";
...
const Projects = () => {
  ...
  return project ? (
    <div
      className="flex flex-col w-full bg-gray-900
        text-white h-screen"
    >
      <div className="flex max-sm:flex-col sm:items-center gap-4, px-4 py-2 no-scrollbar">
          ...
          <div className="sm:hidden flex-1 flex justify-end">
            {isMenuOpen ? (
              ...
              <XIcon
                ...
                className="sieze-6 cursor-pointer"
              ></XIcon>
            )}
          </div>
        </div>
        ...
        <div className="hidden sm:flex gap-2 bg-gray-950 p-1.5 rounded-md">
            <SmartphoneIcon onClick={() => setDevice('phone')} className={`size-6 p-1 rounded cursor-pointer ${device === 'phone' ? 'bg-gray-700' : ''}`}></SmartphoneIcon>
            
            <TabletIcon onClick={() => setDevice('tablet')} className={`size-6 p-1 rounded cursor-pointer ${device === 'tablet' ? 'bg-gray-700' : ''}`}></TabletIcon>
           
            <LaptopIcon onClick={() => setDevice('desktop')} className={`size-6 p-1 rounded cursor-pointer ${device === 'desktop' ? 'bg-gray-700' : ''}`}></LaptopIcon>
        </div>
        {/* right */}
      ...
      <div className="flex-1 flex overflow-auto">
        <Sidebar isMenuOpen={isMenuOpen} project={project} setProject={(p)=> setProject(p)} isGenerating={isGenerating} setIsGenerating={setIsGenerating}></Sidebar>
